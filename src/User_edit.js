...
const schemaPass = yup.object({
    currpass: yup.string().min(2,"Password is too short").max(15,'Password is too long').required('Enter your current password'),
    password: yup.string().min(2,"Password is too short").max(15,'Password is too long').required('Enter your new password'),
        ...
        <LayoutMyAccount>
                        ...
                        <Row>
                            <Col>
                                <Form.Control name="email" type="email" value={values.email} onChange={handleChange}
                                    isInvalid={touched.email && !!errors.email} placeholder="Wpisz nowy email"/>
            ...
            <Formik 
                ...
                onSubmit={(values,errors) => {
                    ...
                    fetch('/chpass', {
                        ...
                        body: JSON.stringify({"oldpass": values.currpass, "newpass": values.password})
                    }).then(res => {
                        if(res.ok) {
            ...
            {({ handleSubmit, handleChange, values, isValid, errors, touched }) => (
                <Form onSubmit={handleSubmit} noValidate>
                    <Row className="mb-3">
                        <Col>
                            <Form.Control name="currpass" type="password" value={values.currpass} onChange={handleChange}
                    ...
                    <Row>
                        <Col>
                            <Form.Control name="passwordConfirmation" type="password" value={values.passwordConfirmation} onChange={handleChange}
